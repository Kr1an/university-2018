...
def main(argv=None):
    ...
    if encrypt:
        with open(filename) as f:
            text = f.read()
        s = BELCRYPT(key, mode=mode)
        with open(filename + '_encrypted', "wb") as f:
            f.write(s.encrypt(text, S=synchro))

    else:
        with open(filename, 'rb') as f:
            data = f.read()
        s = BELCRYPT(key, mode=mode)
        with open(filename[:-10] + "_decrypted", "w") as f:
            f.write(s.decrypt(data, S=synchro))
